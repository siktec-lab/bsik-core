...
class GenerateForm
{
    ...
    # suppress validation error messages and only display your own
    ...
    public static function defined_css($key = '')
    {
        if ($key) {
            return self::$css_config[$key];
        } else {
    ...
    protected function _attributes($data)
    {
        ...
        if (in_array($data['type'], $this->_input_types('button'))) {
            ...
            if (self::defined_css('button')) {
                $string .= 'class="' . self::defined_css('button') . ' ';
            }
        }
        ...
        if ($this->_check_required($data['name'], $data)) {
            ...
            if ($this->in_errors($data['name']) && $this->wrapper != 'bootstrap') {
                $classes .= self::defined_css('text-error') . ' ';
            }
            ...
            if (!empty(self::defined_css('input')))
            {
                ...
                if(isset($data['checkbox-inline']) || ($this->type_is_checkbox($data) && !isset($data['checkbox-inline']))) {
                    if (strpos($this->wrapper, 'bootstrap') !== false) {
                        $return .= ' class="' . self::defined_css('form-check-input') . '"';
                    }
                }
                ...
                if ($data['type'] == 'submit' || $data['type'] == 'button') {
                    if (strpos($this->wrapper, 'bootstrap') !== false) {
                        if(!$data['string']) {
                            $return .= ' class="' . self::defined_css('button-primary') . '"';
                        } else {
                            $return .= ' class="' . self::defined_css('button') . '"';
                        }
                    }
                }
            }
    ...
    protected function _build_input_groups($data)
    {
        ...
        if ($this->is_in_brackets($data['value'])) {
            ...
            if ($this->wrapper == 'bootstrap') {
                ...
                $return .= '<label class="' . self::defined_css('label') . '">' . $data['label'] . '</label>' . $this->_nl(1);
            } else {
                # 'regular' label
            ...
            if (is_array($additional_fields)) {
                ...
                foreach ($additional_fields as $key => $value) {
                    ...
                    if ($this->wrapper == 'bootstrap') {
                        $return .= $this->_t(1) . '<div class="' . self::defined_css($data['type']) . '">';
                    }
    ...
    public function messages($open_tag = '', $close_tag = '')
    {
        ...
        if ($this->inline_errors == false) {
            ...
            if (empty($open_tag) && empty($close_tag)) {
                $open_tag  = '<div class="' . self::defined_css('alert-e') . '">';
                $close_tag = '</div>';
            }
                ...
                if (empty($this->error_messages)) {
                    ...
                    foreach ($this->errors as $key => $value) {
                        if ($this->link_errors == true) {
                            ...
                            $return .= '<a href="#' . $key . '" class="' . self::defined_css('link') . '">' . $value . '</a><br>';
                        } else {
                            # print the message
                            if(strpos($value, '|')) {
    ...
    public function warning_message($str, $flash = false)
    {
        ...
        $return  = '<div class="' . self::defined_css('alert-w') . '" role="alert">';
    ...
    public function success_message($str, $flash = false)
    {
        ...
        $return  = '<div class="' . self::defined_css('alert-s') . '" role="alert">';
        $return .= '    <button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>';
        $return .=      $str;
        $return .= '</div>';
        $this->message = $return;
    ...
    public function error_message($str, $flash = false)
    {
        ...
        $return  = '<div class="' . self::defined_css('alert-e') . '" role="alert">';
        $return .= '    <button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>';
        $return .=      $str;
        $return .= '</div>';
        $this->message = $return;
    ...
    public function info_message($str, $flash = false)
    {
        ...
        $return  = '<div class="' . self::defined_css('alert-i') . '" role="alert">';
        $return .= '    <button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>';
        $return .=      $str;
        $return .= '</div>';
        $this->message = $return;
    ...
    protected function _button($data)
    {
        ...
        if (empty($data['fastform'])) {
            return $this->wrapper($return, $data);
        } else {
    ...
    protected function _create_select($data)
    {
        ...
        if (empty($data['fastform'])) {
            if (!$this->wrapper) {
                ...
                # wrap the element
                $element .= $return;
                return $this->wrapper($element, $data);
            }
        } else {
            # we're using fastform(), which will run the element through wrapper()
            return $return;
    ...
    protected function _create_label($data)
    {
        ...
        if ($this->in_errors($data['name']) && $this->wrapper == 'bootstrap') {
            $return .= ' class="' . self::defined_css('text-error') . '"';
        }
